# Udacity cloud track
This is a list of notes, resources and tutorials for Udacity's first phrase of cloud track scholarship challenge

## AWS
* [VPC](./01_create_vpc)
* [EC2](./02_create_ec2_with_storage)
* [Cleanup VPC & EC2](./03_cleanup)
* [Lamdba](./04_lambda)
* [Elastic Beanstalk](./05_elastic_beanstalk)
* [DynamoDb](./06_dynamo_db)
* [RDS](./07_rds)
* [Cloudfront](./08_cloud_front)
* [IAM](./09_iam)
* [Autoscaling](./10_autoscaling)
* [SNS](./11_sns)
* [Cloudwatch](./12_cloudwatch)
* [Cloudformation UI](./13_cloudformation_ui)
* [IAM for Cloudformation](./14_iam)
* [Cloudformation CLI](./15_cloudformation_cli)
* [Networking infrastructure](./21_networking_infrastructure)

